...
This package provides a benchmark tool for URL downloading processes. It simulates the downloading of files from URLs and measures the performance of the process.
...
```bash
python -m pip install pypdl
```
## Usage

### Basic Usage

Here's a basic example of how to use the benchmark:
...
async def main():
    results = await run_benchmark("test.db", 100, "/tmp/downloads")
    print(results)
